...
namespace KHAS {
	...
	public:
		...
		void addDataItem(const std::string& item) noexcept;
		...
		std::string dividingLine() const noexcept override;
	...
	private:
		...
		std::string createString(TString&& text, Aligment aligment = Aligment::CENTER, char del = '_') const noexcept;

		template <typename TLString, typename TRString>
			requires std::is_convertible_v<std::string, std::decay_t<TLString>>
		&& std::is_convertible_v<std::string, std::decay_t<TRString>>
			std::string createString(TLString&& left, TRString&& right, Aligment aligment, char del) const noexcept;
